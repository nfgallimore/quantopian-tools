...
def create_api_rst(modules, path):
...
modules[package_name].append("__project_name__")
modules[package_name].append("__project_description__")
modules[package_name].append("__project_url__")
...
create_api_rst(modules, os.path.join(basedir, 'docs', 'api.rst'))
...
project = package.__project_name__
author = '{} <{}>'.format(package.__author__, package.__author_email__)
copyright = "{}, {}".format(datetime.strptime(package.__release_date__, '%m/%d/%Y').year, author)
description = package.__project_description__
...
htmlhelp_basename = ''.join(project.split()) + 'Documentation'
...
latex_documents = [
    (
        ...
        ''.join(project.split()) + '.tex',  # Target name
        project + ' Documentation',  # Title
        author.replace('<', '\\textless{}').replace('>', '\\textgreater{}'),  # Author
...
texinfo_documents = [
    (
        master_doc,  # Source start file
        ''.join(project.split()),  # Target name
        project + ' Documentation',  # Title
        author,  # Author
        ''.join(project.split()),  # Dir menu entry
        description.split('\n')[0],  # Description
